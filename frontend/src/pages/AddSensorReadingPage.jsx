...
export default function AddSensorReadingPage() {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      localStorage.setItem("last_assessment", JSON.stringify(data));
      localStorage.setItem("last_assessment_at", new Date().toISOString());
  ...
  useEffect(() => {
    try {
      ...
      const savedAt = localStorage.getItem("last_assessment_at");
      if (saved) setResult(JSON.parse(saved));
      if (savedAt) setLastAssessedAt(new Date(savedAt));
    } catch {
  ...
  return (
    <section className="panel p-5">
      <div className="flex items-center gap-3 mb-4">
        ...
        <img className="page-illustration ml-auto" src={iconUpload} alt="Add sensor reading" />
      </div>
        ...
        <article>
          <div className="flow-card mb-4">
            ...
            <p className="text-sm mt-1 text-slate-700">
              Use these tips to understand how alarms are triggered and how the model scores risk.
            </p>
            <ul className="mt-3 space-y-2 text-sm text-slate-600">
              <li>1. Gas &gt; 250 ppm or temperature &gt; 90C will trigger an alarm.</li>
              <li>2. Smoke &gt; 15 raises severity and increases risk score quickly.</li>
              <li>3. Try a normal case first, then increase one sensor to see the rule hit.</li>
            </ul>
            <button
              type="button"
            ...
            >
              {formOpen ? "Close Form" : "Add Reading"}
            </button>
          </div>
          {formOpen ? (
            ...
            <div className="rounded-xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-600">
              Click “Add Reading” to enter a reading and generate a new assessment.
            </div>
          )}
        </article>
          ...
          <div className="flow-card">
            ...
            {!result ? (
              <p className="text-sm text-slate-500">
                Submit a reading to view alarm decision, risk score, severity, and reason.
              </p>
            ) : (
              <div className="space-y-4">
                <div className="text-xs text-slate-500">
                  {lastAssessedAt ? `Last assessment: ${lastAssessedAt.toLocaleString()}` : "Last assessment saved"}
                </div>
